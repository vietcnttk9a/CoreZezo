...
namespace PMS.Tests
{
    ...
    /// Provides methods to easily work with <see cref="PMSDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<PMSTestModule>
    {
        ...
        {
            void NormalizeDbContext(PMSDbContext context)
            {
        ...
        protected void UsingDbContext(Action<PMSDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PMSDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PMSDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PMSDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PMSDbContext> action)
        {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PMSDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PMSDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PMSDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PMSDbContext, Task<T>> func)
        {
            T result;
